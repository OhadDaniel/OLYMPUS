...
type Phase = "interview" | "wheel" | "goals" | "bindings" | "scan" | "bridge" | "sealed";
...
const WHEEL_LABELS: Record<GodId, string> = {
  zeus: "COUNSEL",
  athena: "CAREER",
  asclepius: "HEALTH",
  hermes: "TASKS",
...
export function FirstMeeting({ onComplete }: { onComplete?: () => void }) {
  ...
  const [msgs, setMsgs] = useState<Msg[]>([
    { role: "zeus", text: "I am Maxwell. Before I can help you fight for your week, I must know you — truly. Let us begin, unhurried. What is your name?" },
  ]);
  ...
  const saveWheel = async () => {
    setBusy(true);
    await post("/onboarding/wheel", { values: Object.fromEntries(WHEEL_GODS.map((g) => [g, wheel[g]])) });
    setBusy(false);
    setPhase("goals");
  };
  const saveGoals = async () => {
    setBusy(true);
    if (goals.length) await post("/onboarding/goals", { goals });
    setBusy(false);
    setPhase("bindings");
  };
  const doScan = async () => {
    setBusy(true);
    try {
      const r = await post("/onboarding/scan-email");
      setInsights(Array.isArray(r.insights) ? r.insights : []);
    } catch {
      /* ignore */
    }
    setScanned(true);
  ...
  const confirmInsight = async (ins: Insight) => {
    const id = ins._id ?? ins.id;
    if (id) await post(`/onboarding/insights/${id}/confirm`);
    setInsights((xs) => xs.map((x) => (x === ins ? { ...x, handled: true } : x)));
  };
  const buildBridge = async () => {
    setBusy(true);
    const r = await post("/onboarding/bridge");
    setBridge(r);
    setBusy(false);
  };
  const sealWeek = async () => {
    setBusy(true);
    if (bridge?.proposalId) await post(`/proposals/${bridge.proposalId}/approve`);
    await post("/onboarding/complete", { name: name || "Ohad", timezone: Intl.DateTimeFormat().resolvedOptions().timeZone });
    setBusy(false);
    setPhase("sealed");
  };
  ...
  const ghost = (t: string, onClick: () => void, disabled = false) => (
    <button
      ...
      style={{ fontSize: 12, letterSpacing: "0.3em", color: GOLD, background: "transparent", border: "1px solid rgba(198,161,91,.45)", padding: "12px 26px", borderRadius: 2, opacity: disabled ? 0.4 : 1, cursor: disabled ? "not-allowed" : "pointer" }}
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-void font-body" style={{ color: IVORY }}>
      <img
        ...
        style={{ filter: "grayscale(.35) brightness(.5) saturate(.7)", opacity: 0.34, animation: "mxOlympusIn 3.6s var(--ease) .3s both" }}
      />
      <div className="pointer-events-none absolute inset-0" style={{ background: "radial-gradient(120% 90% at 50% 30%, transparent 12%, rgba(11,10,16,.9) 74%)" }} />
      <div className="mx-contours mx-contours-ivory" style={{ left: "-16%", top: "6%", width: 900, height: 900, opacity: 0.16 }} />
      <div className="mx-grain" style={{ opacity: 0.06 }} />

      <div className="absolute z-[2] hidden lg:block" style={{ left: "4%", bottom: "8%", width: 360, animation: "mxRise 2.2s var(--ease) 1.4s both" }}>
        <div style={{ animation: "mxDrift 18s ease-in-out infinite" }}>
          <Statue src="zeus-monument.png" god={GODS_DESIGN.zeus} height={360} treatment="dark" />
        </div>
        <div className="mt-3 flex items-center justify-center gap-3">
          <div style={{ width: 34, height: 1, background: "rgba(198,161,91,.45)" }} />
          {label("ZEUS", { letterSpacing: "0.5em" })}
          <div style={{ width: 34, height: 1, background: "rgba(198,161,91,.45)" }} />
        </div>
      </div>

      <div className="absolute inset-x-0 top-0 z-10 flex items-start justify-between px-11 py-8" style={{ animation: "mxRise 1.2s var(--ease) .6s both" }}>
        <div className="flex flex-col gap-2">
          {label("THE FIRST MEETING", { letterSpacing: "0.52em" })}
          <span className="font-voice italic" style={{ fontSize: 14, color: dim(0.55) }}>a rite of twenty-five minutes — unhurried</span>
        </div>
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 flex flex-wrap justify-center gap-7 px-11 py-6">
        {PHASES.map(([p, t]) => {
          ...
          return (
            <span key={p} className="font-label" style={{ fontSize: 10, letterSpacing: "0.24em", color: p === phase ? "#E8C87E" : cur > me ? "rgba(198,161,91,.5)" : dim(0.28) }}>{t}</span>
          );
        })}
      </div>

      <div className="absolute right-[5%] top-1/2 z-[5] w-[620px] max-w-[92vw] -translate-y-1/2" style={{ animation: "mxRise 1.4s var(--ease) 1s both" }}>
        {phase === "interview" && (
          <div className="flex flex-col gap-6">
            <div ref={scroller} className="flex max-h-[52vh] flex-col gap-5 overflow-y-auto pr-2">
              {msgs.map((m, i) =>
                m.role === "zeus" ? (
                  <p key={i} className="font-voice italic" style={{ fontSize: 23, lineHeight: 1.45, color: IVORY }}>{m.text || "…"}</p>
                ) : (
                  <p key={i} className="self-end" style={{ fontSize: 15, color: dim(0.6), background: "rgba(237,230,214,.06)", padding: "8px 14px", borderRadius: 2 }}>{m.text}</p>
                ),
              )}
            </div>
            <input
              ...
              placeholder="offer your answer…"
              disabled={streaming}
              style={{ width: "100%", background: "transparent", border: "none", borderBottom: `1px solid ${dim(0.28)}`, outline: "none", fontFamily: "var(--font-voice)", fontStyle: "italic", fontSize: 24, color: IVORY, padding: "10px 2px", caretColor: GOLD }}
            />
            <div className="flex items-center gap-5">
              {ghost("OFFER —", () => void send(), streaming || !input.trim())}
              <span className="ml-auto font-label" style={{ fontSize: 9, letterSpacing: "0.26em", color: dim(0.4) }}>EACH ANSWER KINDLES A STAR</span>
              {coverage >= 3 && ghost("THE WHEEL —", () => setPhase("wheel"))}
            </div>
          </div>
        )}
            ...
            <div className="flex flex-col gap-4">
              {WHEEL_GODS.map((g) => (
                <div key={g} className="flex items-center gap-4">
                  <span className="font-label" style={{ width: 90, fontSize: 10, letterSpacing: "0.22em", color: GODS_DESIGN[g].hue }}>{WHEEL_LABELS[g]}</span>
            ...
            {goals.length < 3 && (
              <div className="flex items-center gap-3">
                <select value={goalGod} onChange={(e) => setGoalGod(e.target.value as GodId)} className="bg-void2 font-label" style={{ color: GODS_DESIGN[goalGod].hue, border: `1px solid ${dim(0.2)}`, padding: 10, fontSize: 11 }}>
                  {GOD_ORDER.filter((g) => g !== "zeus").map((g) => (
                    <option key={g} value={g}>{GODS_DESIGN[g].name}</option>
                  ))}
                </select>
                <input
                  value={goalDraft}
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter" && goalDraft.trim()) {
                      setGoals((xs) => [...xs, { godId: goalGod, title: goalDraft.trim() }]);
                      setGoalDraft("");
                    }
                  }}
                  placeholder="name a labor…"
                  style={{ flex: 1, background: "transparent", borderBottom: `1px solid ${dim(0.28)}`, outline: "none", fontFamily: "var(--font-voice)", fontStyle: "italic", fontSize: 18, color: IVORY, padding: "8px 2px" }}
                />
              </div>
            )}
        ...
        {phase === "bindings" && (
          <div className="flex flex-col gap-6">
            ...
            {[
              { k: "google", bound: google, on: () => connectGoogle(), t: "GOOGLE — CALENDAR & INBOX" },
              { k: "telegram", bound: telegram, on: () => void connectTelegram(), t: "TELEGRAM — THE MESSENGER" },
            ].map((b) => (
                ...
                {insights.length === 0 ? (
                  ...
                  insights.map((ins, i) => (
                    <div key={i} className="flex items-center gap-3" style={{ border: `1px solid ${dim(0.16)}`, padding: "14px 16px", opacity: ins.handled ? 0.45 : 1 }}>
                      <span style={{ fontSize: 15 }}>{ins.summary}</span>
                      {!ins.handled ? (
                        <button onClick={() => void confirmInsight(ins)} className="ml-auto font-label" style={{ fontSize: 10, letterSpacing: "0.2em", color: GOLD, cursor: "pointer" }}>CARVE —</button>
                      ) : (
                        <span className="ml-auto font-label" style={{ fontSize: 10, color: GOLD }}>✓ CARVED</span>
                      )}
                    </div>
                  ))
                )}
        ...
        {phase === "bridge" && (
          <div className="flex flex-col gap-6">
            ...
            {!bridge ? (
              ...
              <>
                <span className="font-voice italic" style={{ fontSize: 18, color: dim(0.72) }}>
                  {bridge.summary?.adds ? `${bridge.summary.adds} blocks, woven around your real week — approve to make them real.` : "A gentle start, around the outer world."}
                </span>
      ...
      {phase === "sealed" && (
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-8" style={{ background: "rgba(245,242,234,.97)" }}>
          ...
          <h1 className="font-display" style={{ fontWeight: 400, fontSize: 64, color: "#14131A", textAlign: "center" }}>
            Welcome to Olympus, <span className="italic" style={{ color: GOLD }}>{name || "Ohad"}</span>.
          </h1>
